...
config = dict(
    ...
    optimizer_reducer_compression=0.27,
...
def main():
    ...
    model_param = f"/mydata/ckpt/resnet_model_weights_new_{config['dataset']}.pth"

    if not os.path.exists(model_param):
        print(f"File does not exist. Saving model weights to {model_param}.")
        torch.save(task._model.state_dict(), model_param)
    else:
        print(f"File already exists at {model_param}. Skipping save.")

    # 加载模型权重
    task._model.load_state_dict(torch.load(model_param))
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for i, batch in enumerate(train_loader):
            ...
            if i == 0 and config["optimizer_batch_size"] == 10:
                ...
                torch.save(labels, f'/mydata/rawlabel/labels_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_{sys.argv[4]}.pt')
                #this is photo.
                save_path = f"/mydata/rawlabel/saved_tensor_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_{sys.argv[4]}.pt"
                ...
                plt.savefig(f'/mydata/photo/image_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_{sys.argv[4]}.png', bbox_inches='tight')
            ...
            with timer("batch", epoch_frac):
                ...
                with timer("batch.reduce", epoch_frac):
                    ...
                    torch.save(grads, f'/mydata/gradient/gradient_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_{sys.argv[4]}.pt')
...
if __name__ == "__main__":
    ...
    if config["optimizer_reducer"] == "RankKReducer":
        ...
        if level == 'low':
            config["optimizer_reducer_compression"] = 0.1
        elif level == 'mid':
            config["optimizer_reducer_compression"] = 0.01
        elif level == 'high':
            config["optimizer_reducer_compression"] = 0.001
