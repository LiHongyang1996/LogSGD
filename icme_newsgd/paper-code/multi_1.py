...
config = dict(
    ...
    optimizer_reducer_compression=1.0,
...
def main():
    ...
    for epoch in range(config["num_epochs"]):
        ...
        for i, batch in enumerate(train_loader):
            ...
            with timer("batch", epoch_frac):
                ...
                with timer("batch.reduce", epoch_frac):
                    ...
                    bits_communicated += reducer.reduce(send_buffers, grads, memories, config['quantized'])
        ...
        print(timer.summary())
        log_cache.append(timer.save_summary())

        # if config["rank"] == 0:
        #     timer.save_summary(os.path.join(output_dir, f"/document/{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}.json"))


    timestamp = time.strftime("%Y%m%d_%H%M%S")
    save_logs_to_file(f"document/{config['dataset']}_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_{timestamp}.txt")
    final_save_path = f"checkpoint/final_checkpoint_{config['dataset']}_{sys.argv[1]}_{sys.argv[2]}_{sys.argv[3]}_{timestamp}.pth"
    torch.save(
        {
            "epoch": config["num_epochs"],
            "test_stats": test_stats,
            "model_state_dict": task.state_dict(),
            "runavg_state_dict": runavg_model.value(),
        },
        final_save_path,
    )
    print(f"Final model and run average saved to {final_save_path}")
...
def log_metric(name, values, tags={}):
    """Log timeseries data and store in cache."""
    global log_cache  # Access the global log cache
    ...
    print("{name:30s} - {values} ({tags})".format(name=name, values=values, tags=tags))
    log_entry = "{name:30s} - {values} ({tags})".format(
        name=name, values=values_str, tags=tags_str
    )

    # Append log entry to the global cache
    log_cache.append(log_entry)


def save_logs_to_file(filename):
    """Save all cached logs (including summary) to a file."""
    with open(filename, 'w') as f:
        for log in log_cache:
            # Ensure each log entry is a string before writing
            if isinstance(log, dict):
                # Convert dictionary to a string representation, such as JSON
                f.write(str(log) + "\n")
            else:
                f.write(str(log) + "\n")
...
if __name__ == "__main__":
    ...
    config["optimizer_reducer"] = str(sys.argv[1])
    if str(sys.argv[2])=="quantized":
        config["quantized"] = True
    else: config["quantized"] = False

    level = str(sys.argv[3])

    config['dataset'] = str(sys.argv[4])

    if config["optimizer_reducer"] == "RankKReducer":
        if level == 'low':
            config["optimizer_reducer_rank"] = 7
        elif level == 'mid':
            config["optimizer_reducer_rank"] = 2
        elif level == 'high':
            config["optimizer_reducer_rank"] = 1


    elif config["optimizer_reducer"] == "TopKReducer":
        if level == 'low':
            config["optimizer_reducer_compression"] = 405327360/17163205632
        elif level == 'mid':
            config["optimizer_reducer_compression"] = 126351360/17163205632
        elif level == 'high':
            config["optimizer_reducer_compression"] = 70556160/17163205632

    elif config["optimizer_reducer"] == "SignAndNormReducer":
        pass
